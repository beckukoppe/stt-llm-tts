...
def transcribe_audio():
    try:
        ...
        result = subprocess.run([
            "./whisper/whisper.cpp/build/bin/whisper-cli",  # adjust if needed
            "-m", "./whisper/whisper.cpp/models/ggml-base.en.bin",
            "-f", TEMP_AUDIO_FILE,
            "-otxt"
...
def wait_for_server(timeout=60):
    ...
    print(f"[INFO] Waiting for Coqui TTS server to respond")

    start = time.time()
    while time.time() - start < timeout:
        for url in urls:
            try:
                # We send a dummy POST to see if it's alive — don't need valid content
                response = requests.post(url, timeout=1)
                if response.status_code in (405,):
                    # Server is up and responding — just rejecting bad input
                    print(f"[✅] Server is up and reachable at {url}")
                    return True
            except requests.exceptions.RequestException:
                pass
        time.sleep(0.25)

    print("[❌] Timeout: Coqui TTS server did not become available.")
    return False
